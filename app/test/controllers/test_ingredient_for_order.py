import pytest
from app.controllers import IngredientForOrderController


def test_create(app, beverage_for_order: dict):
    created_beverage_for_order, error = IngredientForOrderController.create(
        beverage_for_order)
...
def test_get_all(app, beverages_for_order: list):
    ...
    for beverage_for_order in beverages_for_order:
        created_beverage_for_order, _ = IngredientForOrderController.create(
            beverage_for_order)
        created_beverage_for_orders.append(created_beverage_for_order)

    beverages_for_order_from_db, error = IngredientForOrderController.get_all()
